...
contract('MultiSigWalletFactory', function(accounts) {
  ...
  describe('create contract', async function() {
    ...
    it('should anyone to create MultiSigWallet contract', async function() {
      ...
      assert.equal(await walletContract.threshold(), 1)
    })
    ...
    it('should not allow to create with lesser fees', async function() {
      ...
      await assertRevert(factory.create(accounts.slice(0, 2), 1, {value: fee}))
    })
    ...
    it('should allow to create with more or same fees', async function() {
      ...
      let receipt = await factory.create(accounts.slice(0, 2), 1, {
        value: newFee
      })
      assert.equal(receipt.logs.length, 1)
      assert.equal(receipt.logs[0].event, 'ContractCreated')
  ...
  describe('withdraw', async function() {
    ...
    it('should allow owner to withdraw fund', async function() {
      await assertRevert(factory.create(accounts.slice(0, 2), 1))
      ...
      for (let i = 0; i < testData.length; i++) {
        ...
        let createReceipt = await factory.create(
          accounts.slice(0, 2),
          1,
          testData[i]
        )
    ...
    it('should not allow other to withdraw fund', async function() {
      ...
      let createReceipt = await factory.create(accounts.slice(0, 2), 1, {
      ...
      await assertRevert(factory.withdraw({from: accounts[2]}))
      await assertRevert(factory.withdraw({from: accounts[6]}))
